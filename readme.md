...
Бот для Telegram, который отвечает на сообщения **не всегда**, а с заданной вероятностью. Использует локальную модель через [Ollama](https://ollama.com) и не требует интернета для генерации ответов. Идеально подходит для уютных чатов, где хочется偶尔 видеть остроумные комментарии от ИИ.
